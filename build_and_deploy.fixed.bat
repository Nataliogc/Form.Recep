...
set "DEPLOY=%~dp0deploy_frontend_DCO.fixed.bat"
...
REM 2) Construir (si tienes build)
...
REM 3) Copiar estáticos a /dist (si procede)
if not exist dist mkdir dist
for %%I in (index.html live.html live.mobile.html) do (
  if exist "%%I" copy /y "%%I" "dist\%%I" >nul
)

REM 4) Commit de cambios locales antes del deploy
git add -A
git commit -m "%MSG%" 1>nul 2>nul

REM 5) Lanzar deploy (bump ?v= y push)
call "%DEPLOY%"
popd
