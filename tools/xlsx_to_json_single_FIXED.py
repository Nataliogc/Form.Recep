# -*- coding: utf-8 -*-
# Generador Excel -> JSON con "departamento"
# Uso: python tools/xlsx_to_json_single_FIXED.py --xlsx tu.xlsx
    ...
    for cat,gcat in vdf.groupby("categoria"):
        ...
        for lvl,glvl in gcat.groupby("nivel"):
            ...
            for j,rr in glvl.reset_index(drop=True).iterrows():
                ...
                pool.append({"id":qid,"categoria":cat_name,"department":dept_value,"level":lvl_slug,"text":str(rr["text"]).strip(),"options":options,"correct":options[idx],"correct_letter":str(rr["correct_letter"]).strip().upper(),"why":str(rr["why"]).strip(),"source":str(rr["source"]).strip()})
            with open(os.path.join(OUT,f"{cat_slug}_{lvl_slug}.json"),"w",encoding="utf-8") as f: json.dump(pool,f,ensure_ascii=False,indent=2)
            index_manifest.setdefault(cat_slug,{"name":cat_name})
            index_manifest[cat_slug]["dept"]=dept_value
            index_manifest[cat_slug].setdefault("levels",{})[lvl_slug]={"agg":f"{cat_slug}_{lvl_slug}.json","blocks_dir":f"{cat_slug}/blocks/{lvl_slug}/","count":len(pool),"block_size":BLOCK_SIZE}
